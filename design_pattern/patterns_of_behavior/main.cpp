...
public:
    ...
    void HowManyObserver() { std::cout << "There are " << list_observer_.size() << " observers in the list.\n"; }
...
/*
    观察者模式里有两个对象，一个是被观察者，一个是观察者。
    1.被观察者需要维护一个观察者列表，当被观察者发生变化时，需要通知(Notify)观察者列表里的所有观察者；被观察者需要有添加和删除观察者的方法。
    2.观察者需要实现一个更新方法，当被观察者通知观察者时，观察者需要调用更新方法来更新自己。
*/